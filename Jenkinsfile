pipeline {
    ...
    stages {
        ...
        stage('dev') {
            ...
            steps {
                sh "find . -type f -name '*actionsvc*.jar' -not -name '*docker-info*' -exec mv {} target/actionsvc.jar \\;"
        ...
        stage('test') {
            ...
            steps {
                ...
                sh 'sh artifactory-get.sh -r http://artifactory-sdc.onsdigital.uk/artifactory/libs-release-local/ -g uk.gov.ons.ctp.product -a actionsvc > target/actionsvc.jar'
                sh 'chmod 777 target/actionsvc.jar'
